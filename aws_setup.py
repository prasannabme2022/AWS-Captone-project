...
# ============================================
# MULTIMODAL AI HEALTH PREDICTION ROUTES
# ============================================

@app.route('/doctor/multimodal')
def doctor_multimodal_page():
    """Render the doctor-facing multimodal prediction UI."""
    if session.get('role') not in ('doctor', 'admin'):
        flash('Access restricted to doctors and admins.', 'error')
        return redirect(url_for('login'))
    return render_template('doctor/multimodal_predict.html')


@app.route('/api/multimodal/predict', methods=['POST'])
def api_multimodal_predict():
    """
    Full Multimodal Health Prediction API.
    Consumes JSON body → returns strict prediction JSON.
    Also persists result to DynamoDB and fires SNS on High/Critical.
    """
    if 'user_id' not in session:
        return jsonify({'error': 'Unauthorized'}), 401

    try:
        # ── Import the upgraded ML engine ──────────────────────────────
        ...
        result = predictor.predict_full(
            ...
            s3_image_uri        = data.get('s3_image_uri', ''),
            bp                  = data.get('bp'),
            spo2                = data.get('spo2'),
        ...
        # ── Persist prediction to DynamoDB ─────────────────────────────
        try:
            import json
            pred_record = {
                'vault_id':    generate_id('PRED'),
                'patient_email': data.get('patient_id', 'UNKNOWN'),
                'file_name':   f"AI_Prediction_{result['timestamp']}.json",
                'file_type':   'AI_Prediction',
                'file_path':   'in-system',
                'analysis':    json.dumps(result, ensure_ascii=False)[:3000],
                'uploaded_at': result['timestamp'],
            }
            medical_vault_table.put_item(Item=pred_record)
            logger.info(f"Prediction stored for patient {data.get('patient_id')}")
        except Exception as db_err:
            logger.warning(f"Could not persist prediction to DynamoDB: {db_err}")

        # ── Fire SNS alert if needed ───────────────────────────────────
        if result.get('alert_trigger', {}).get('send_alert'):
            try:
                send_notification(
                    message = result['alert_trigger']['message'],
                    subject = f"[MedTrack {result['alert_trigger']['priority'].upper()} ALERT] "
                              f"Patient {data.get('patient_id','?')} — "
                              f"{result['primary_prediction']['condition']}"
                )
                logger.info(f"SNS alert fired for patient {data.get('patient_id')}")
            except Exception as sns_err:
                logger.warning(f"SNS alert failed: {sns_err}")
